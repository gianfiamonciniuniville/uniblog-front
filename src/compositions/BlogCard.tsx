  ...
  return (
    <Box p={5} shadow="md" borderWidth="1px" borderRadius="lg">
      ...
      <Stack direction="row" gap={4} mt={4}>
        <Link to={`/blogs/${blog.id}`}>
          <Button variant="primary" size="sm"> {/* Use custom primary button */}
            View Details
          </Button>
        </Link>
        {onEdit && (
          <Button variant="secondary" size="sm" onClick={() => onEdit(blog.id)}> {/* Use custom secondary button */}
            Edit
          </Button>
        )}
        {onDelete && (
          <Button variant="danger" size="sm" onClick={() => onDelete(blog.id)}> {/* Use custom danger button */}
            Delete
          </Button>
        )}
      </Stack>
