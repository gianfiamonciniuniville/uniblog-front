...
const PostForm: React.FC<PostFormProps> = ({
  ...
  useEffect(() => {
    const fetchBlogs = async () => {
      try {
        ...
        setBlogs(data);
        if (!initialData && data.length > 0 && !blogId) { // Only set default if creating new post and no blog selected
          setBlogId(data[0].id);
        }
      } catch (err: any) {
    ...
    fetchBlogs();
  }, [initialData, blogId]); // Added blogId to dependency array
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (initialData) {
      ...
      await onSubmit({ title, content, slug, authorId: 0, blogId: blogId as number });
    }
  };
        ...
        <Stack gap={4}>
          ...
          <FormControl id="blogId" isRequired>
            ...
            <Select
              ...
              value={blogId || ""}
              onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setBlogId(Number(e.target.value))}
              disabled={disabled || fetchingBlogs} // initialData is not a reason to disable select
            >
              {blogFetchError && <option value="" disabled>{blogFetchError}</option>}
              {blogs.map((blog) => (
            ...
            {blogFetchError && <Alert status="error" mt={2}><AlertIcon />{blogFetchError}</Alert>}
          </FormControl>
