    ...
    <Box mt={8}>
      ...
      <VStack gap={4} align="stretch">
        {comments.map(comment => (
          <Box key={comment.id} p={4} shadow="sm" borderWidth="1px" borderRadius="md">
            <Flex justifyContent="space-between" alignItems="center">
                <Text fontSize="sm" color="gray.500">
                    <ChakraLink as={RouterLink} to={`/profile/${comment.user.id}`} color="blue.500" fontWeight="bold">
                        {comment.user.userName}
                    </ChakraLink> says:
                </Text>
                {user?.id === comment.user.id && (
                    <Button
                        variant="danger" // Use custom danger variant
                        size="xs"
